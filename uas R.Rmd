...
# Create ROC curve
roc_curve_dt <- multiclass.roc(test_data$LULUS, as.numeric(predictions_dt))
...
```{r}
# Preprocess the data
preProcValues <- preProcess(train_data_binary, method = c("center", "scale"))
trainTransformed <- predict(preProcValues, train_data_binary)
testTransformed <- predict(preProcValues, test_data_binary)
...
knnModel <- train(
  ...
  metric = "Precision"  # Specify precision as the metric for tuning
)

# Extract best k from the tuned model
...
predictions <- predict(best_model, testTransformed, type = "prob")

# Extract probability of the positive class for AUC calculation
probabilities_knn <- predictions[, 2]

# Calculate ROC curve
roc_curve_knn <- roc(test_data_binary$LULUS, probabilities_knn)

# Calculate AUC
auc_value_knn <- auc(roc_curve_knn)

# Display evaluation metrics for K-Nearest Neighbors
